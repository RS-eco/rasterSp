...
#download.file("http://spatial-data.s3.amazonaws.com/groups/REPTILES.zip", destfile=paste0(filedir, "/IUCN/REPTILES.zip"))
...
amphibians %<>% as.data.frame() %>% select(-geometry) %>% group_by(binomial, presence, origin, seasonal, kingdom_na, phylum_nam, 
                                                                   class_name, order_name, family_nam) %>% summarise_at("shape_Area", sum)
...
ter_mammals %<>% as.data.frame() %>% select(-geometry) %>% group_by(binomial, presence, origin, seasonal, kingdom_na, phylum_nam, 
                                                                    class_name, order_name, family_nam) %>% summarise_at("shape_Area", sum)
...
reptiles %<>% as.data.frame() %>% select(-geometry) %>% group_by(binomial, presence, origin, seasonal, kingdom, phylum, 
...
#But, BirdLife provides a checklist for each version of their data product (http://datazone.birdlife.org/species/taxonomy).

#**Note:** I am not sure if the data currently used is Version 7 or older, but here we use checklist v7.
...
birds <- lapply(list.files(paste0(filedir, "/BirdLife"), 
                           pattern=".shp", full.names=TRUE), FUN=function(x){
                             ...
                             data %<>% as.data.frame() %>% select(c("SISID", "SCINAME", "PRESENCE", "ORIGIN", "SEASONAL"))
                             data$area <- area
                             return(data)
                           })
...
file.remove("data/amphibians_dist.csv.xz", "data/ter_mammals_dist.csv.xz", "data/ter_birds_dist.csv.xz", 
            "data/gard_reptiles_dist.csv.xz", "data/reptiles_dist.csv.xz")
...
ter_birds_endemic <- range_birds %>% filter(area <= quantile(range_birds$area, probs=0.15)) %>% data.frame()
save(ter_birds_endemic, file="data/ter_birds_endemic.rda", compress="xz")
